...
insertNewGeneralCategory conn name = void $ execute conn "INSERT INTO categories (name) VALUES (?)" (Only name)
...
insertNewCategory conn name parentCategoryId = void $ execute conn "INSERT INTO categories (name,\"parentId\") VALUES (?,?)" (name, parentCategoryId)
...
editCategory conn name parentCategoryId categoryId = void $ execute conn "UPDATE categories SET (name,\"parentId\") = (?,?) WHERE id = (?)" (name, parentCategoryId, categoryId)
