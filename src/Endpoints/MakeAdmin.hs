...
makeAdmin :: Connection -> LBS.ByteString -> IO Response
makeAdmin conn body = case decode body :: Maybe API.IdRequest of
  ...
  Just bodyParsed -> do
    ...
    case users of
      ...
      [x] -> if isAdmin x
      then pure $ responseBadRequest "This user is already admin"
      else do
        DB.makeAdmin conn editUserId
        pure $ responseOk "Now this user is an admin"